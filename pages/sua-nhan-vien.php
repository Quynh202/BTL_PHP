...
if(isset($_SESSION['username']) && isset($_SESSION['level']))
{
  ...
  if(isset($_GET['id']))
  {
    ...
    $nv_id = $row['loai_cb_id'];
    $ten_nv = $row['ten_loai_cb'];
    ...
    $lnv = "SELECT id, ten_loai_cb FROM loai_cb WHERE id <> $nv_id";
  ...
  // chuc nang tu dong update hang luong va bac luong cua can bo 
  $hangCanBo = $row['hang_cb']; 
  $bacLuong = $row['bac_nhom_ngach']; 
  $maCanBo = $row['ma_cb'];
  $idCanBo = $row['id']; 

  $showData1 = "SELECT ngay_bat_dau, nam_tang_luong_min FROM cong_tac WHERE cong_tac.canbo_id = $idCanBo"; 
  $result1 = mysqli_query($conn, $showData1);
  $row1 = mysqli_fetch_array($result1);
  $ngayBatDau = $row1['ngay_bat_dau']; 
  $namTangLuong = $row1['nam_tang_luong_min']; 
  if($namTangLuong < $ngayBatDau){
    $namTangLuong = $ngayBatDau; 
  }
  
  $showData2 = "SELECT loai_kt_id FROM khen_thuong_ky_luat kyluat WHERE kyluat.canbo_id = $idCanBo"; 
  $showDataFlag = "SELECT flag FROM khen_thuong_ky_luat kyluat WHERE kyluat.canbo_id = $idCanBo"; 
  $result2 = mysqli_query($conn, $showData2); 

  if ($result2=mysqli_query($conn,$showData2)){
    // Return the number of rows in result set
      $rowcount=mysqli_num_rows($result2);
      if($rowcount>0){
        $row2 = mysqli_fetch_array($result2);
        $loai_kt_id = $row2['loai_kt_id']; 
      }else {
        $loai_kt_id = ''; 
      }
  }
  ...
  $result3 = mysqli_query($conn, $showDataFlag); 

  if ($result3=mysqli_query($conn,$showDataFlag)){
  ...
  $namTl = date_create($namTangLuong);  
  if($flag== '' && $loai_kt_id== ''){
    date_add($namTl, date_interval_create_from_date_string('3 years')); 
  } else {
    if($loai_kt_id==14){
      date_add($namTl, date_interval_create_from_date_string('3 months')); 
    } else if($loai_kt_id==15){
      date_add($namTl, date_interval_create_from_date_string('6 months')); 
    } else if($loai_kt_id==19){
      date_add($namTl, date_interval_create_from_date_string('1 year')); 
    }
  }
  date_format($namTl, 'Y-m-d'); 
  if($namTl == date('Y-m-d'));{
    $bacLuong = $bacLuong + 1; 
      if($bacLuong > 9){
        $hangCanBo = $hangCanBo - 1; 
        if($hangCanBo <=0){
          $bacLuong = $bacLuong - 1; 
          $hangCanBo = $hangCanBo +1; 
        }
  ...
  if(isset($_POST['save']))
  {
    ...
    if($trinhDo == 'chon')
      $error['trinhDo'] = 'error';
    if($bacLuong == "Nhập bậc nhóm ngạch")
      $error['bacLuong'] = 'error'; 
    ...
    if(!$error)
    {
      ...
      else
      {
        ...
        $update = " UPDATE can_bo SET 
                    ...
                    WHERE id = $id";
        $result = mysqli_query($conn, $update);
        $update1 = "UPDATE cong_tac SET nam_tang_luong_min = $namTangLuong"; 
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Chỉnh sửa thông tin cán bộ </li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Chỉnh sửa thông tin cán bộ</h3> &emsp;
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="exampleInputEmail1" name="maNhanVien" value="<?php echo $row['ma_cb']; ?>" readonly>
                    </div>
                    <div class="form-group">
                      <label>Tên cán bộ <span style="color: red;">*</span>: </label>
                      <input type="text" class="form-control" id="exampleInputEmail1" placeholder="Nhập tên nhân viên" name="tenNhanVien" value="<?php echo $row['ten_cb']; ?>">
                    ...
                    <div class="form-group">
                      ...
                      <?php 
                        ...
                        {
                          echo "<option value='".$lnv['id']."'>".$lnv['ten_loai_cb']."</option>";
                        }
                      ?>
                      </select>
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      <label>Hạng cán bộ: <span style="color: red;">*</span>: </label>
                      <?php echo $hangCanBo; ?>
                      <small style="color: red;"><?php if(isset($error[''])){ echo "Vui lòng nhập hạng cán bộ"; } ?></small>
                    </div>

                    <div class="form-group">
                      <label>Bậc nhóm ngạch: <span style="color: red;">*</span>: </label>
                      <?php echo $bacLuong; ?>
                      <small style="color: red;"><?php if(isset($error[''])){ echo "Vui lòng nhập bậc lương"; } ?></small>
                    </div>
