<?php 
    ...
    $sheet->setCellValue('W' . $rowCount, 'Trạng thái');
  	...
  	// do du lieu tu db
  	$sql = "SELECT cb.id as id, ma_cb, hinh_anh, ten_cb, biet_danh, gioi_tinh, cb.ngay_tao as ngay_tao, ngay_sinh, noi_sinh, so_cmnd, ten_tinh_trang, ngay_cap_cmnd, noi_cap_cmnd, nguyen_quan, ten_quoc_tich, ten_dan_toc, ten_ton_giao, ho_khau, tam_tru, ten_loai_nv, ten_trinh_do, ten_chuyen_mon, ten_bang_cap, ten_phong_ban, ten_chuc_vu, trang_thai FROM canbo cb, quoc_tich qt, dan_toc dt, ton_giao tg, loai_cb lcb, trinh_do td, chuyen_mon cm, bang_cap bc, phong_ban pb, chuc_vu cv, tinh_trang_hon_nhan hn WHERE cb.quoc_tich_id = qt.id AND cb.dan_toc_id = dt.id AND cb.ton_giao_id = tg.id AND cb.loai_nv_id = lcb.id AND cb.trinh_do_id = td.id AND cb.chuyen_mon_id = cm.id AND cb.bang_cap_id = bc.id AND cb.phong_ban_id = pb.id AND cb.chuc_vu_id = cv.id AND cb.hon_nhan_id = hn.id ORDER BY cb.id DESC";
      ...
      else
      {
        $trangThai = 'Đã nghỉ việc';
  		...
  		$sheet->setCellValue('A' . $rowCount, $stt);
	  	$sheet->setCellValue('B' . $rowCount, $row['ma_cb']);
	  	$sheet->setCellValue('C' . $rowCount, $row['ten_cb']);
      ...
      $sheet->setCellValue('Q' . $rowCount, $row['ten_loai_cb']);
